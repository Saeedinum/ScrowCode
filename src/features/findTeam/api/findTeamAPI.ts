...
export const findTeamAPI = createApi({
  ...
  endpoints: builder => ({
    fetchTeams: builder.query({
      ...
      transformResponse: (response: { CompletedOrNo: BACKEND_T_teams[]; pendingSentMe: BACKEND_T_teams[]; pendingIsent: BACKEND_T_teams[] }) => {
        const teams = [
          ...
          ...[...response.pendingIsent, ...response.pendingSentMe].map(item => {
            const team = {
              id: item._id,
              name: {
                english: item.projectNameEnglish,
