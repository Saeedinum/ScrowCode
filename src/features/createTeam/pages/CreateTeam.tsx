...
const CreateTeam = () => {
  ...
  const isLastMemberFilled = teamMembers?.length ? teamMembers[teamMembers.length - 1]?.username?.trim() : true
    ...
    if (user?.token) {
      ...
      if ("error" in response) {
        response.error.data.errors.map(e => {
          ...
          if (e.path === "userName") {
            e.value.map(a => {
              const index = getValues("teamMembers").findIndex(e => e.username === a)
  ...
  return (
    <main className="max-w-screen relative flex select-none justify-start overflow-hidden">
      ...
      <section className="flex w-1/2 flex-grow flex-col items-center justify-start px-5 py-0 font-bold">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-10 flex w-full flex-col items-start">
          <div className="w-full">
            <h2 dir="rtl" className="text-[20px] text-primary-first">
              1. تعيين تفاصيل المشروع
            </h2>
            <div dir="rtl" className="mb-5 mt-5 flex flex-wrap items-center justify-start gap-4">
              <label dir="rtl" htmlFor="projectArabicName" className="relative w-[285px] text-sm md:w-[329px]">
                <span className="mr-1 text-primary-first">اسم المشروع باللغه العربية</span>
                <input
                  autoComplete="false"
                ...
                />
                {errors.projectArabicName && <span className="text-red-500">{errors.projectArabicName.message} </span>}
              </label>
              <label dir="rtl" htmlFor="projectEnglishName" className="relative w-[285px] text-sm md:w-[329px]">
                <span className="mr-1 text-primary-first">اسم المشروع باللغه الانجليزية</span>
                <input
                  autoComplete="false"
                ...
                />
                {errors.projectEnglishName && <span className="text-red-500">{errors.projectEnglishName.message} </span>}
              </label>
              <label dir="rtl" htmlFor="projectCategorie" className="relative w-[285px] text-sm md:w-[329px]">
            ...
            <div dir="rtl">
              <label htmlFor="projectDescription" className="relative h-40 w-full">
                ...
                ></textarea>
                {errors.projectDescription && <span className="text-red-500">{errors.projectDescription.message}</span>}
              </label>
            </div>
          </div>
          ...
          <div dir="rtl" className="w-full">
            <div>
              <h2 className="flex items-end gap-2 text-[20px] text-primary-first">
                ...
                <span className="mr-1 text-[12px] font-[600] text-[#5D6A93]">قم بادخال ما تريد من التراكات بما سيخص مشروعك</span>
              </h2>
              ...
              <div className="my-6 grid grid-cols-1 gap-5 text-nowrap sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {tracks?.map((track: Ttracks, index) => (
                  <div key={track._id} className="relative my-5 flex items-center">
                    <input {...register(`requirement.${index}.trackID`)} type="checkbox" id={track.slug} value={track._id} className="peer hidden" />
                    <label htmlFor={track.slug} className="relative flex cursor-pointer items-center text-primary-second">
                    ...
                    <Track styles="hidden peer-checked:flex" index={index} handleNeeds={handleTrackNeeds} />
                  </div>
                ))}
              </div>
            </div>
          ...
          <span className="my-5 h-[1px] w-[calc(100%-5rem)] place-self-center bg-[#BCBCBC]"></span>
          <section dir="rtl" className="flex w-full flex-col items-start justify-start">
            <h2 className="text-[20px] text-primary-first">3.اضافة اعضاء التيم</h2>
            <div dir="rtl" className="mb-5 mt-5 flex flex-wrap items-center justify-start gap-4">
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
              ...
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                <span className="mr-1 text-primary-first">اسم المستخدم علي سكرو</span>
                <input
                  type="text"
                  disabled
                  value={user.username!}
              ...
              <p className="flex w-fit flex-col items-start font-[600]">
                ...
                <span className="text-[#5D6A93]">العضو الأول هو المسوؤل عن التيم</span>
                <span className="text-[#A0A1A3]">يمكنك تغيير المسوؤل لاحقا</span>
              </p>
            </div>
            <div className="mb-5 mt-5 w-full">
              {fields.map((field, index) => (
                <div key={field.username} className="relative m-2 flex flex-row flex-wrap gap-4 p-1">
                  <label key={field.id} htmlFor="teamMembers" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                    ...
                    />
                    {errors.teamMembers?.[index]?.username && <span className="text-red-500">{errors.teamMembers?.[index]?.username.message}</span>}
                  </label>
                  <svg onClick={() => remove(index)} className="mt-6 cursor-pointer self-center" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="9" cy="9" r="9" className="hover:fill-red-600" fill="#5D6A93" />
                    <path
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <svg width="25" height="25" viewBox="0 0 25 25" fill="primary-third" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="12.5" cy="12.5" r="12.5" fill="white" />
                  <path d="M19.7913 13.5413H13.5413V19.7913H11.458V13.5413H5.20801V11.458H11.458V5.20801H13.5413V11.458H19.7913V13.5413Z" fill="#6694FF" />
                </svg>
                اضافة عضو
              </button>
              <p className="flex w-fit flex-col items-start font-[600]">
                <span className="text-primary-first"> لاحظ :</span>
                <span className="text-[#5D6A93]">عدد الطلاب لا يزيد عن 8 ولا يقل عن 5</span>
                <span className="font-[700] text-[#5D6A93]">يمكنك اضافة اقل من ذلك في حالة ان التيم ليس مكتمل</span>
              </p>
            </div>
          </section>
          <span className="my-5 h-[1px] w-[calc(100%-5rem)] place-self-center bg-[#BCBCBC]"></span>
          <section dir="rtl" className="mb-5 mt-5 flex w-full flex-wrap gap-4 place-self-start">
            <h2 className="text-[20px] text-primary-first">4.اضافة المشرفين علي المشروع</h2>
            <div className="mr-2 mt-1 flex w-full flex-wrap gap-4 place-self-start">
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                <span className="mr-1 text-primary-first">اسم المشرف الرئيسي ( عضو هيئة التدريس )</span>
                <input
                  type="text"
                  {...register("supervisor", {
                    required: true
                ...
                />
                <span className="text-xs font-[500] text-[#5D6A93]">اختياري اذا لم تحدد المشرف الرئيسي</span>
              </label>
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                <span className="mr-1 text-primary-first">اسم المشرف المساعد ( معيد أو مدرس مساعد )</span>
                <input
                  type="text"
                ...
                />
                <span className="text-xs font-[500] text-[#5D6A93]">اختياري اذا لم تحدد المشرف المساعد</span>
              </label>
            </div>
          </section>
          <button type="submit" className="relative my-10 h-[49px] w-[calc(70%)] place-self-center text-nowrap rounded-[8px] bg-[#002ABA] text-primary-fourth duration-500 hover:bg-primary-first">
            {isLoading ? (
              <p
