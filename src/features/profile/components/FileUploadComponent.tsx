...
const FileUploadComponent = () => {
  ...
  return (
    <label {...getRootProps()} className="dashed mt-7 flex h-[253px] w-[271px] cursor-pointer flex-col items-center justify-center gap-2 rounded-[10px]" htmlFor="file">
      {file ? (
        <p className="text-xl font-bold text-primary-first">{file.name}</p>
      ) : (
        <>
          <img src={uploadIcon} alt="Upload icon" />
          <p className="text-[20px] font-semibold text-primary-first">ارفع الملف هنا</p>
          <p className="text-xs font-semibold text-[#5D6A93]">أقصي حجم 20 ميحا</p>
        </>
      )}
