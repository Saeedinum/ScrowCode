...
const Team = () => {
  ...
  const handleDeleteUser = (id: string) => {
    if (getValues("deletedMembers")) setValue("deletedMembers", [...getValues("deletedMembers"), id])
  ...
  const onSubmit: SubmitHandler<TEditTeamData> = data => {
    const requirements = getValues()
      ...
      .flat()
    const { addedElements, removedElements } = compareArrays(team.tracks.map(t => t.members.map(m => m.id)).flat(), requirements)
    data = {
      ...data,
  ...
  useEffect(() => {
    if (!isLoading && data) {
      ...
      setValue("teamMembers", data.tracks.map(t => t.members.map(m => m.id)).flat())
    }
  ...
  if (!isDesktop) {
    return (
      <div className="flex items-center justify-center">
        <p className="mt-10 text-center font-serif text-red-500">This feature is currently available for desktop only, as some functionalities are not supported on mobile devices.</p>
      </div>
    )
  }
  ...
  return (
    <>
      <Helmet>
        <title>التيم الخاص بي</title>
        <meta name="description" content="التيم الخاص بي" />
      </Helmet>
      {isLoading && <LoadingDialog />}

      <form onSubmit={handleSubmit(onSubmit)} dir="rtl" className="w-full px-20 pt-10">
        <section className="flex flex-col items-start">
          <h1 className="text-[29px] font-bold text-primary-first">التيم الخاص بي</h1>
        ...
        <section className="flex w-full flex-col items-start">
          <section>
            ...
            <div className="flex flex-wrap gap-5">
              <label dir="rtl" htmlFor="projectArabicName" className="relative flex w-[285px] flex-col text-sm md:w-[329px]">
                ...
                />
                <span className="text-xs font-medium text-[#5D6A93]">اسم المشروع لابد ان يكون اسم علمي وليس تجاري</span>
              </label>
              <label dir="rtl" htmlFor="projectEnglishName" className="relative flex w-[285px] flex-col text-sm md:w-[329px]">
              ...
              <label dir="rtl" htmlFor="projectCategorie" className="relative flex w-[285px] flex-col text-sm md:w-[329px]">
            ...
            <div className="flex w-full justify-between">
              <div className="">
                <label htmlFor="projectDescription" className="relative flex max-h-[400px] min-h-[150px] flex-col">
                ...
                <div className=" ">
                  <h2 className="text-[20px] font-bold text-[#6694FF]">متطلبات التيم</h2>
                      ...
                      {tracks?.map((track: Ttracks, index) => (
                        <div key={track._id} className="relative my-5 flex items-center">
                          <input
                            ...
                            type="checkbox"
                            defaultChecked={team.tracks.some(teamNeed => teamNeed.id === track._id)}
                            id={track.slug}
                            value={track._id}
                          ...
                          <Track
                            ...
                            index={index}
                            min={team.tracks.find(teamNeed => teamNeed.id === track._id)?.members.length as number}
                        ...
                        <div key={track.id} className="flex items-center gap-4">
                          <p
                            className={`h-[27px] w-[26px] rounded-[5px] bg-[#EEF3FF] text-center text-[20px] font-semibold text-[#5D6A93] ${
                              track.maxmembers - track.members.length === 0 ? "" : "border-2 border-red-500 pb-1"
                            } `}
                          >
                            {track.maxmembers}
                          </p>
                          <p className="flex flex-col text-[20px] font-semibold text-primary-first">
                            {track.name}
                            <span className="text-sm font-semibold text-[#5D6A93]">{track.maxmembers - track.members.length === 0 ? `مكتمل` : `متبقي ${track.maxmembers - track.members.length}`}</span>
                          </p>
                        </div>
                      ))}
                    </div>
              ...
              <div className="mt-10">
                ...
                <p className="flex flex-col text-xs font-medium text-[#5D6A93]">
                  <span>الملف يكون عبارة عن document يحتوي علي تفاصيل المشروع كاملا </span>
          ...
          <section>
            <h2 className="text-[20px] font-semibold text-[#6694FF]">اعضاء التيم</h2>
            <p className="flex gap-3 text-sm text-primary-first">
              <span>عدد الاعضاء 8</span>
              <span>مكتمل :{team.members.current}</span>
              {team.members.current === 8 ? "" : <span className="text-red-500">باقي : {8 - team.members.current}</span>}
            </p>
                ...
                <div className="flex flex-col">
                  {track.members.map(member => (
                    <div className="flex items-center gap-4">
                      {editMode && <UserDropDown handleDeleteUser={handleDeleteUser} token={user.token!} id={member.id} />}
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="" className="size-[30px] fill-Grey-first">
                        <path
                          fillRule="evenodd"
                      ...
                      <div className="flex flex-col items-center">
                        <p className="flex flex-col font-semibold text-primary-first">{member.name}</p>
                        <p className="text-sm font-semibold text-[#407BFF]">{track.name}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {Array(8 - team.members.current)
                ...
                .map((_, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="" className="size-[20px] fill-Grey-first">
                      <path
                        fillRule="evenodd"
                        d="M18.685 19.097A9.723 9.723 0 0 0 21.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 0 0 3.065 7.097A9.716 9.716 0 0 0 12 21.75a9.716 9.716 0 0 0 6.685-2.653Zm-12.54-1.285A7.486 7.486 0 0 1 12 15a7.486 7.486 0 0 1 5.855 2.812A8.224 8.224 0 0 1 12 20.25a8.224 8.224 0 0 1-5.855-2.438ZM15.75 9a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z"
                        clipRule="evenodd"
                    ...
                    <p className="text-sm font-semibold text-[#B4B4B4]">لم يتم الانضمام</p>
                  </div>
                ))}
            </div>
          </section>
          ...
          <section dir="rtl" className="mb-5 mt-5 flex w-full flex-wrap gap-4 place-self-start">
            <h2 className="text-[20px] font-bold text-[#6694FF]">المشرفين علي المشروع</h2>
            <div className="mr-2 mt-1 flex w-full flex-wrap gap-4 place-self-start">
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                <span className="mr-1 font-medium text-primary-first">اسم المشرف الرئيسي ( عضو هيئة التدريس )</span>
                <input
                  disabled={!editMode && team.admin}
                  type="text"
              ...
              <label dir="rtl" className="relative w-[285px] text-sm font-[500] md:w-[329px]">
                <span className="mr-1 font-medium text-primary-first">اسم المشرف المساعد ( معيد أو مدرس مساعد )</span>
                <input
                  disabled={!editMode && team.admin}
                  type="text"
