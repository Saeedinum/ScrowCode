...
import { useGetMyProfileQuery, useUpdateMyProfileMutation } from "../api/profileAPI.ts"
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="الملف الشخصي" />
      </Helmet>
      {isLoading && <LoadingDialog />}
      <form onSubmit={handleSubmit(onSubmit)} dir="rtl" className="mt-7 flex w-full flex-col gap-3 px-2 lg:px-20">
        <section className="flex w-full justify-between">
          <div className="flex gap-5">
            <div className="flex flex-col items-center">
              <img src="/src/assets/global/userProfileIcon.webp" alt="" className="rounded-[8px] lg:h-[147px] lg:w-[156px]" />
              {data?.team ? (
                <p className="mt-1 flex gap-2 text-nowrap">
                  <img src={avialableIocn} alt="" className="size-[20px]" />
                  <span className="font-medium text-green-600"> منضم الي تيم</span>
                </p>
              ) : (
                <p className="mt-1 flex gap-2 text-nowrap">
            ...
            <div>
              <h1 className="text-[20px] font-medium text-primary-first">{data?.user.fullName}</h1>
              ...
              <div className="max-md:absolute">
                ...
                <div className="top-0 flex flex-wrap gap-3">
                  {data?.mySkills.map(skill => (
                    <div key={skill} className={`relative flex h-[40px] w-fit items-center text-nowrap rounded-[8px] border border-[#407BFF] bg-[#E1EBFF] p-[10px] text-sm lowercase text-[#407BFF]`}>
          ...
          {pathname === "/myProfile" && (
            <>
              {editMode ? (
                <button type="submit" className="flex h-[37px] items-center justify-center gap-2 rounded-[8px] border-[1px] border-solid bg-primary-first px-3 text-[15px] text-white">
                  حفظ
                </button>
              ) : (
                <button
        ...
        <section className="flex flex-wrap justify-start gap-10">
          <section className="p-4">
            ...
            <div className="mr-3 mt-3 w-[367px] rounded-[8px] border border-[#95A3D5] px-[14px] py-[9px] font-medium text-[#5D6A93]">empty</div>
            ...
            <div className="mt-2 flex flex-col gap-5">
                ...
                {editMode ? (
                  ...
                  <>
                    {data?.contact?.linkedin ? (
                      <a href={`https://linkedin.com/in/${data?.contact?.linkedin}`} className="cursor-pointer font-semibold text-[#5D6A93] hover:text-primary-second">
                        {data?.contact?.linkedin}
                      </a>
                    ) : (
                      "not provided"
              ...
              <div className="flex items-center gap-1">
                ...
                {editMode ? (
                  ...
                  <>
                    {data?.contact?.github ? (
                      <a href={`https://github.com/${data?.contact?.github}`} className="cursor-pointer font-semibold text-[#5D6A93] hover:text-primary-second">
                        {data?.contact?.github}
                      </a>
                    ) : (
                      "not provided"
                ...
                {editMode ? (
                  ...
                  <>
                    {data?.contact?.behance ? (
                      <a href={`https://behance.net/${data?.contact?.behance}`} className="cursor-pointer font-semibold text-[#5D6A93] hover:text-primary-second">
                        {data?.contact?.behance}
                      </a>
                    ) : (
                      "not provided"
