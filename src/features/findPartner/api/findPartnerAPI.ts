...
export const findPartnerAPI = createApi({
  ...
  endpoints: builder => ({
    getStudents: builder.query({
      ...
      transformResponse: (response: { AvilableOrNot: BACKEND_T_partners[]; pendingISent: BACKEND_T_partners[]; pendingSentMe: BACKEND_T_partners[]; status: string }) => {
        const partners: Tpartner[] = [
          ...response.AvilableOrNot.map(items => {
            return {
              ...
              status: items.Available ? "available" : ("notAvailable" as Tpartner["status"]),
          ...
          ...[...response.pendingISent, ...response.pendingSentMe].map(items => {
            return {
              id: items._id,
              name: items.user?.fullName || "",
              imageURL: items.user?.profileImg,
...
export const { useGetStudentsQuery, useSendOrderToStudentMutation } = findPartnerAPI
